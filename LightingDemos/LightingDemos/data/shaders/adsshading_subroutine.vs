...
subroutine uniform ShadeModelType uni_shademode;
...
vec3 DiffuseOnly(vec4 position, vec3 norm)
{
	...
	return (uni_light.Ld * uni_material.Kd * max(dot(s, norm), 0.0));
}
...
void main()
{
	...
	// Light intensity out
	vary_light_intensity = uni_shademode(eyeCoords, tnorm);
