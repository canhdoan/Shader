...
void Torus::Construct(float outerRadius, float innerRadius, int nsides, int nrings)
{
    rings = nrings; 
    sides = nsides;
    
	m_nFaces = sides * rings;
    ...
    // Elements
    unsigned int * el = new unsigned int[6 * m_nFaces];
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, 6 * m_nFaces * sizeof(unsigned int), el, GL_STATIC_DRAW);
...
void Torus::Render(void)
{
	glBindVertexArray(m_nVAO);
    glDrawElements(GL_TRIANGLES, 6 * m_nFaces, GL_UNSIGNED_INT, ((GLubyte *)NULL + (0)));
}
